...
class BasicBlock(spconv.SparseModule):
    ...
    def forward(self, x):
        ...
        out = self.layers(x)
        output = spconv.SparseConvTensor(identity.features + out.features,
...
class SemanticBranch(nn.Module):
    def __init__(self, sizes=(256, 256, 32), nbr_class=19, init_size=32, class_frequencies=None, phase="train"):
        super().__init__()
    ...
    @staticmethod
    def bev_projection(vw_features, vw_coord, sizes, batch_size) -> torch.Tensor:
    ...
    def forward_once(self, vw_features, coord_ind, full_coord, pw_label, info):
        ...
        proj1_vw, vw1_coord, pw1_coord = self.proj1_block(info, conv1_output.features, output_scale=2,
                                                          input_coords=coord.int(), input_coords_inv=full_coord)
        ...
        proj1_bev_rwkv = self.rwkv_block1(proj1_bev.permute(0, 2, 3, 1).reshape(B, H * W, C),
                                          patch_resolution=patch_resolution).view(B, H, W, C).permute(0, 3, 1, 2)

        conv2_input_tensor = spconv.SparseConvTensor(proj1_vw, vw1_coord.int(),
                                                     (np.array(self.sizes, np.int32) // 2)[::-1], batch_size)
        conv2_output = self.conv2_block(conv2_input_tensor)
        proj2_vw, vw2_coord, pw2_coord = self.proj2_block(info, conv2_output.features, output_scale=4,
                                                          input_coords=vw1_coord.int(), input_coords_inv=pw1_coord)
        proj2_bev = self.bev_projection(proj2_vw, vw2_coord, (np.array(self.sizes, np.int32) // 4)[::-1], batch_size)
        B, C, H, W = proj2_bev.shape
        patch_resolution = (H, W)
        proj2_bev_rwkv = self.rwkv_block2(proj2_bev.permute(0, 2, 3, 1).reshape(B, H * W, C),
                                          patch_resolution=patch_resolution).view(B, H, W, C).permute(0, 3, 1, 2)

        conv3_input_tensor = spconv.SparseConvTensor(proj2_vw, vw2_coord.int(),
                                                     (np.array(self.sizes, np.int32) // 4)[::-1], batch_size)
        conv3_output = self.conv3_block(conv3_input_tensor)
        proj3_vw, vw3_coord, _ = self.proj3_block(info, conv3_output.features, output_scale=8,
                                                  input_coords=vw2_coord.int(), input_coords_inv=pw2_coord)
        proj3_bev = self.bev_projection(proj3_vw, vw3_coord, (np.array(self.sizes, np.int32) // 8)[::-1], batch_size)
        B, C, H, W = proj3_bev.shape
        patch_resolution = (H, W)
        proj3_bev_rwkv = self.rwkv_block3(proj3_bev.permute(0, 2, 3, 1).reshape(B, H * W, C),
                                          patch_resolution=patch_resolution).view(B, H, W, C).permute(0, 3, 1, 2)

        if self.phase in {"train", "val"}:
            index_02 = torch.cat(
                [info[2]["bxyz_indx"][:, 0].unsqueeze(-1), torch.flip(info[2]["bxyz_indx"], dims=[1])[:, :3]], dim=1)
            index_04 = torch.cat(
                [info[4]["bxyz_indx"][:, 0].unsqueeze(-1), torch.flip(info[4]["bxyz_indx"], dims=[1])[:, :3]], dim=1)
            index_08 = torch.cat(
                [info[8]["bxyz_indx"][:, 0].unsqueeze(-1), torch.flip(info[8]["bxyz_indx"], dims=[1])[:, :3]], dim=1)
    ...
    def forward(self, data_dict, example):
        if self.phase in {"train", "val"}:
            out_dict = self.forward_once(data_dict["vw_features"],
                data_dict["coord_ind"], data_dict["full_coord"], example[M.LABEL_LIDAR.value], data_dict["info"])
            all_teach_pair = out_dict["mss_logits_list"]
            device = all_teach_pair[0][0].device

            class_weights = self.get_class_weights(device=device, dtype=data_dict["vw_features"].dtype)
            loss_dict = {}
...
class SemanticBranch2D(SemanticBranch):
    @staticmethod
    def get_class_weights(device, dtype) -> torch.Tensor:
        """
        Eye-ball class weights
        """
        return torch.tensor([2.0, 2.0, 3.0], device=device, dtype=dtype)
