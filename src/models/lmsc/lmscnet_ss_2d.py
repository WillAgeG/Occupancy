...
class LMSCNet_SS_2d(nn.Module):
    ...
    def forward(self, x):
        ...
        out_scale_1_1__2D_head = self.seg_head_1_1(out_scale_1_1__2D)

        # Take back to [W, H, D] axis order
        out_scale_1_1__2D_head = out_scale_1_1__2D_head  # [bs, C, H, W, D] -> [bs, C, W, H, D]

        scores = {"pred_semantic_1_1": out_scale_1_1__2D_head}
    ...
    def compute_loss(self, scores, data):
        ...
        """
        target = data["2D_LABEL"]["1_1"]
        device, dtype = target.device, target.dtype
        class_weights = self.get_class_weights().to(device=device, dtype=torch.float32)

        criterion = nn.CrossEntropyLoss(weight=class_weights, ignore_index=255, reduction="mean").to(device=device)

        loss_1_1 = criterion(scores["pred_semantic_1_1"], target.long())
