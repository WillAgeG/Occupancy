...
def _get_remap_lut(config_path):
    ...
    dataset_config = yaml.safe_load(pathlib.Path(config_path).open())
...
def _read_calib_SemKITTI(calib_path):
    ...
    """
    calib_all = {}
    with pathlib.Path(calib_path).open() as f:
        for line in f:
            if line == "\n":
                break
...
def get_remap_lut(path):
    ...
    dataset_config = yaml.safe_load(pathlib.Path(path).open())
...
def get_mask(pc, lims):
    mask_x = mask_op(pc[:, 0], lims[0] + 0.0001, lims[3] - 0.0001)
    mask_y = mask_op(pc[:, 1], lims[1] + 0.0001, lims[4] - 0.0001)
    mask_z = mask_op(pc[:, 2], lims[2] + 0.0001, lims[5] - 0.0001)
