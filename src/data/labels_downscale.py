...
def majority_pooling(grid, k_size=2):
    ...
    for xx in range(int(np.floor(grid.shape[0] / k_size))):
        for yy in range(int(np.floor(grid.shape[1] / k_size))):
            for zz in range(int(np.floor(grid.shape[2] / k_size))):
...
def main():
    ...
    remap_lut = SemanticKittiIO.get_remap_lut(
        os.path.join(yaml_path, "semantic-kitti.yaml")
    )
    dataset_config = yaml.safe_load(
        pathlib.Path(os.path.join(yaml_path, "semantic-kitti.yaml")).open("r")
    )
    ...
    for sequence in sequences:
        ...
        for i in range(len(label_paths)):
            ...
            LABEL = np.moveaxis(
                LABEL.reshape(
                    [grid_dimensions[0], grid_dimensions[2], grid_dimensions[1]]
                ),
                [0, 1, 2],
                [0, 2, 1],
            )  # [256, 32, 256]
            ...
            for scale in downscaling:
                ...
                if not (
                    pathlib.Path(label_filename).is_file() & pathlib.Path(invalid_filename).is_file()
                ):
                    LABEL_ds, INVALID_ds = downscale_data(LABEL, downscaling[scale])
                    SemanticKittiIO.pack(INVALID_ds.astype(dtype=np.uint8)).tofile(
                        invalid_filename
                    )
                    print(
                        time.strftime("%x %X")
                        + " -- => File {} - Sequence {} saved...".format(
                            filename + ".label_" + scale, os.path.basename(sequence)
                        )
                    )
                    ...
                    print(
                        time.strftime("%x %X")
                        + " -- => File {} - Sequence {} saved...".format(
                            filename + ".invalid_" + scale, os.path.basename(sequence)
                        )
                    )

        print(
            time.strftime("%x %X")
            + f" -- => All files saved for Sequence {os.path.basename(sequence)}"
        )
