...
class BEVUNetv1(nn.Module):
    def __init__(self, n_class, _n_height, dilation, bilinear, group_conv,
                  input_batch_norm, dropout, circular_padding, dropblock):
        super().__init__()
        # Encoder
        ...
        # Decoder
        self.up1 = Up(1024, 512, circular_padding, bilinear=bilinear, group_conv=group_conv,
                       use_dropblock=dropblock, drop_p=dropout)
        self.up2 = Up(768, 256, circular_padding, bilinear=bilinear, group_conv=group_conv,
                       use_dropblock=dropblock, drop_p=dropout)
        self.up3 = Up(384, 128, circular_padding, bilinear=bilinear, group_conv=group_conv,
                       use_dropblock=dropblock, drop_p=dropout)
        self.up4 = Up(192, 128, circular_padding, bilinear=bilinear, group_conv=group_conv,
                       use_dropblock=dropblock, drop_p=dropout)
    ...
    def forward(self, x, sem_fea_list, com_fea_list):
        ...
        x2 = self.down1(x1)
        B, C, H, W = x2.shape
        x2 = self.rwkv_block1(x2.permute(0, 2, 3, 1).reshape(B, H * W, C),
                               patch_resolution=(H, W)).view(B, H, W, C).permute(0, 3, 1, 2)
        x2_f = self.bev_fusions[0](x2, sem_fea_list[0], com_fea_list[0])

        x3 = self.down2(x2_f)
        B, C, H, W = x3.shape
        x3 = self.rwkv_block2(x3.permute(0, 2, 3, 1).reshape(B, H * W, C),
                               patch_resolution=(H, W)).view(B, H, W, C).permute(0, 3, 1, 2)
        x3_f = self.bev_fusions[1](x3, sem_fea_list[1], com_fea_list[1])

        x4 = self.down3(x3_f)
        B, C, H, W = x4.shape
        x4 = self.rwkv_block3(x4.permute(0, 2, 3, 1).reshape(B, H * W, C),
                               patch_resolution=(H, W)).view(B, H, W, C).permute(0, 3, 1, 2)
        x4_f = self.bev_fusions[2](x4, sem_fea_list[2], com_fea_list[2])

        x5 = self.down4(x4_f)
        B, C, H, W = x5.shape
        x5 = self.rwkv_block4(x5.permute(0, 2, 3, 1).reshape(B, H * W, C),
                               patch_resolution=(H, W)).view(B, H, W, C).permute(0, 3, 1, 2)
...
class DoubleConv(nn.Module):
    """(conv => BN => ReLU) * 2"""
    def __init__(self, in_ch, out_ch, group_conv, _dilation=1):
        super().__init__()
        if group_conv:
...
class DoubleDonvCircular(nn.Module):
    """(conv => BN => ReLU) * 2"""
    def __init__(self, in_ch, out_ch, group_conv, _dilation=1):
        super().__init__()
        if group_conv:
        ...
        if input_batch_norm:
            if circular_padding:
                self.conv = nn.Sequential(
                    ...
                    DoubleDonvCircular(in_ch, out_ch, group_conv=False, _dilation=dilation)
                )
            else:
                self.conv = nn.Sequential(
                    nn.BatchNorm2d(in_ch),
                    DoubleConv(in_ch, out_ch, group_conv=False, _dilation=dilation)
                )
        elif circular_padding:
            self.conv = DoubleDonvCircular(in_ch, out_ch, group_conv=False, _dilation=dilation)
        else:
            self.conv = DoubleConv(in_ch, out_ch, group_conv=False, _dilation=dilation)
    ...
    def __init__(self, in_ch, out_ch, dilation, group_conv, circular_padding):
        ...
        if circular_padding:
            self.mpconv = nn.Sequential(
                nn.MaxPool2d(2),
                DoubleDonvCircular(in_ch, out_ch, group_conv=group_conv, _dilation=dilation)
            )
        else:
            self.mpconv = nn.Sequential(
                nn.MaxPool2d(2),
                DoubleConv(in_ch, out_ch, group_conv=group_conv, _dilation=dilation)
            )
...
class Up(nn.Module):
    def __init__(self, in_ch, out_ch, circular_padding, bilinear=True,
                 group_conv=False, use_dropblock=False, drop_p=0.5):
        super().__init__()
        ...
        if circular_padding:
            self.conv = DoubleDonvCircular(in_ch, out_ch, group_conv=group_conv)
        else:
            self.conv = DoubleConv(in_ch, out_ch, group_conv=group_conv)
