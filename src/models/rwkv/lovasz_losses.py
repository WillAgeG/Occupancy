...
def lovasz_grad(gt_sorted):
    ...
    p = len(gt_sorted)
    gts = gt_sorted.sum()
    intersection = gts - gt_sorted.float().cpu().cumsum(0).to(gt_sorted.device, non_blocking=True)
    union = gts + (1 - gt_sorted).float().cpu().cumsum(0).to(gt_sorted.device, non_blocking=True)
...
def xloss(logits, labels, ignore=None):
    ...
    return F.cross_entropy(logits.cpu(), Variable(labels).cpu(), ignore_index=255).to(device)
...
def jaccard_loss(probas, labels, ignore=None, smooth=100, bk_class=None):
    ...
    true_1_hot = true_1_hot.to(vprobas.device, non_blocking=True)
