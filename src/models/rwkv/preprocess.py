...
class PcPreprocessor(nn.Module):
    def __init__(self, lims, sizes, grid_meters, init_size=32, offset=0.5, pooling_scales=(0.5, 1, 2, 4, 6, 8),
        ...
        input_size = 10 if timestamp else 9
        self.multi_scale_top_layers = nn.ModuleDict()
        self.feature_list = {
            0.5: [input_size, init_size],
            1: [input_size, init_size],
        }
        self.target_scale = 1
        for scale in self.feature_list:
    ...
    def add_pcmean_and_gridmean(self, pc, bxyz_indx, return_mean=False):
        ...
        xmin_ymin_zmin = torch.tensor(
            [self.lims[0][0], self.lims[1][0], self.lims[2][0]], dtype=torch.float, device=pc.device)
    ...
    def forward(self, pc, indicator):
        indicator_t = []
        tensor = torch.ones((1,), dtype=torch.long).to(pc, non_blocking=True)
        ...
        self.sizes = self.sizes.to(pc, non_blocking=True)
